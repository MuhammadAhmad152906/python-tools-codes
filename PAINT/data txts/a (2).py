...
class UltimatePaint:
    def __init__(self, root):
        ...
        # Canvas - Exactly 1400x900
        self.canvas = tk.Canvas(root, bg="white", width=1400, height=900)
        ...
        # Control panel frame
        control_frame = tk.Frame(root, pady=2)
        ...
        tool_frame = tk.Frame(root, pady=2)
        ...
        # Style selector + preview frame
        style_frame = tk.Frame(root, pady=2)
        style_frame.pack(fill=tk.X)
        
        # Style OptionMenu Setup
        self.style_var = tk.StringVar(value=self.current_style)
        style_keys = list(self.styles.keys())
        
        # Explicit menu definition to guarantee visibility
        self.style_menu = tk.OptionMenu(style_frame, self.style_var, *style_keys, command=lambda _: self.update_preview())
        self.style_menu.config(width=25)  # Set explicit width so it's clearly visible
        self.style_menu.pack(side=tk.LEFT, padx=5)
        
        self.preview_canvas = tk.Canvas(style_frame, bg="white", width=200, height=40, highlightthickness=1, highlightbackground="gray")
        self.preview_canvas.pack(side=tk.LEFT, padx=5)
        self.update_preview()

        # Template buttons frame
        template_frame = tk.Frame(root, pady=2)
    ...
    def update_preview(self):
        ...
        style=self.styles[self.style_var.get()]
        self.preview_canvas.create_line(10,20,190,20,width=self.pen_size,fill=self.pen_color,
